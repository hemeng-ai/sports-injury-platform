...
- **文件管理**：拖拽上传、批量上传、搜索筛选、标签筛选、预览下载、软删除、批量操作
- **树状目录**：递归文件夹树、右键菜单、展开收起、权限控制
- **指标体系**：指标 CRUD、分类管理、Excel 自动映射、Excel 导出
- **Excel 解析**：上传解析 Sheet/表头/数据列、自动识别指标类型
- **统计分析**：描述统计（mean/max/min/stdev）、Pearson 相关性、线性趋势、卡片网格展示
- **图表**：散点图、折线图、柱状图、趋势线、迷你 Sparkline
- **权限管理**：三级角色（Visitor/Admin/SuperAdmin）、RBAC 路由守卫
- **操作审计**：操作日志记录与查看（超级管理员可见）
- **UI 体验**：暗色玻璃态主题、数字滚动动画、骨架屏加载、路由进度条、Toast 通知统一
